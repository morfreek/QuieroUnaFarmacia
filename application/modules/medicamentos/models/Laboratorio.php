<?php
class Medicamentos_Model_Laboratorio extends Zend_Db_Table_Abstract{
    ...
    public function listar($opcion=array()){
        $sql = $this->select()->setIntegrityCheck(false)
                ->from(array('LAB'=>'LABORATORIO'),array('LAB_ID','LAB_NOMBRE'))
                ->order(array("LAB_NOMBRE ASC"));

        $lista = array();
        foreach($this->fetchAll($sql) as $resultado){
            $aux = new stdClass();
            $aux->id = $resultado->LAB_ID;
            $aux->nombre = $resultado->LAB_NOMBRE;
            $lista[] = $aux;
        }
        return $lista;
    }
